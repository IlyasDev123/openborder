...
class PaymentService
{
    ...
    public function paymentSubscription($request)
    {
        ...
        if ($request->stripe_payment_method) {
            $this->attachUserToPaymentMethod($request);
        }
        ...
        try {
            $response = $this->stripe->subscriptions->create([
                ...
                'default_payment_method' => $user->stripe_payment_method,
    ...
    public function addCard($request)
    {
        ...
        if ($data->save()) {
            ...
            return sendSuccess("Add card successfully", $data);
        }
        return sendError('Fail to add card', null);
    }
