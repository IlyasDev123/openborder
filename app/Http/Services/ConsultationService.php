...
use App\Notifications\ConsultancyNotification;
...
class ConsultationService
{
    ...
    public function bookConsultation($request)
    {
        ...
        try {
            ...
            // Mail::send('emails.consultation-request', $dataView, function ($m) use ($request) {
            //     $m->from(env("MAIL_FROM_ADDRESS"), env('MAIL_FROM_NAME'));
            //     $m->to(Auth::user()->email)->subject('Consultation Request');
            // });
            ...
            // ];
            // $d = collect($consultation_booking)->except('questionnaire_summery');
            // dispatch(new SendDataToZapier($consultation_booking->questionnaire_summery, $d, $userDetail));
            // Notification::send($user, new ConsultancyNotification($dataView));
        } catch (\Exception $e) {
            DB::rollback();
    ...
    public function formFields($request, $immigrationType, $factorOption)
    {
        if ($request->appointmentTypeID == 16496767) {
            ...
            return  [
                ...
                ["id" => 9776898, "value" => $immigrationType], //$request->description
