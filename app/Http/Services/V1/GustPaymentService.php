...
class GustPaymentService
{
    ...
    public function updateCard($request)
    {
        ...
        if (is_null($user->stripe_id)) {
            return sendError("You didn't buy any service please buy the service to update the card.", null);
        }

        $this->stripe->customers->update($user->stripe_id, ['source' => $request->stripe_token]);

        return sendSuccess("Your payment method has been updated.", $user);
    }
    /**
     * @param mixed $error
     *
    ...
    public function getUser($request)
    {
        ...
        $user = User::where('email', $request->email)->where("is_guest", 1)->first();
        if (!$user) {
            return sendApiError("You have an account registered under this e-mail. Please login using that account to update your payment method.", null);
        }
        return sendSuccess('success', $user);
    }
}
